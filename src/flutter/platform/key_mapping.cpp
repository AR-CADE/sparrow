...
static const uint64_t kValueMask    = 0x000ffffffffULL;
static const uint64_t kUnicodePlane = 0x00000000000ULL;
static const uint64_t kXkbPlane     = 0x01500000000ULL;

static inline uint64_t apply_id_plane(uint64_t id, uint64_t plane)
{
    return (id & kValueMask) | plane;
}

static inline uint64_t to_lower(uint64_t n)
{
    return ((n >= 'A') && (n <= 'Z')) ? (n + ('a' - 'A')) : n;
}

static const std::unordered_map<uint64_t, uint64_t> xkb_to_physical_key_map = {
...
static const std::unordered_map<uint64_t, uint64_t> xkb_keysym_to_logical_key_map = {
    ...
    {0x0000ffeb, 0x00200000106}, // Super_L -> metaLeft
    {0x0000ffec, 0x00200000107}, // Super_R -> metaRight
...
{
    auto found = xkb_to_physical_key_map.find(xkb_keycode);
    if (found != xkb_to_physical_key_map.end())
    {
        return found->second;
    }

    return apply_id_plane(xkb_keycode, kXkbPlane);
}

uint64_t sparrow_keysym_to_logical_key(xkb_keysym_t sym)
{
    auto found = xkb_keysym_to_logical_key_map.find(sym);
    if (found != xkb_keysym_to_logical_key_map.end())
    {
        return found->second;
    }
    ...
    if (sym < 256)
    {
        return apply_id_plane(to_lower(sym), kUnicodePlane);
    }

    return apply_id_plane(sym, kXkbPlane);
}
